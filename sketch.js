...
var gameOverImg,cycleBell, gameOverBgm;
...
function preload(){
  ...
  bgm = loadSound("Grand Score bgm.mp3");
  ...
  gameOverBgm = loadSound("gameOver.wav");
  ...
  var select_oppPlayer = Math.round(random(1,5));
   ...
   if(pinkCG.isTouching(mainCyclist)){
    ...
    if(obs1.isTouching(mainCyclist)){
      gameState = END;
      obst1.velocity = 0;
      obst1.addAnimation("obstacleOne",obstacle1);
      gameOverBgm.play();
    }
    ...
    if(obs2.isTouching(mainCyclist)){
      gameState = END;
      obst2.velocity = 0;
      obst2.addAnimation("obstacleTwo",obstacle2);
      mainCyclist.remove();
      gameOverBgm.play();
    }
    ...
    textSize(20);
    fill(255);
    text("Press Space Bar to Restart the game!", 500,200);
    ...
    obs1.setVelocityEach(0);
    obs1.setLifetimeeach(-1);

    obs2.setVelocityEach(0);
